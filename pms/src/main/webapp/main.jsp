...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/skin/css/main.css" />
	...
	<script type="text/javascript">
        ...
        $(function(){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/notice/getLimitList',
                dataType:'json',
                success:function(resultData){
                    $(resultData).each(function(index,item){
                    	var notice = JSON.stringify(item);
                        var dateInfo= new Date(item.ndate);
                        var di= dateInfo.getFullYear()+"-"+(dateInfo.getMonth()+1)+"-"+dateInfo.getDate();
						 var li ="<li class='ue-clear'><span>"+di+"</span>&nbsp;&nbsp;&nbsp;<a href='javascript:showWindow("+notice+")'class='notice-title'>"+item.ntitle+"</a></li><p>";
                        $("#notices").append(li);
                    });
                },
                error:function(){
			...
			$.ajax({
				type:'GET',
				url:'${pageContext.request.contextPath}/tz/getTieZiList',
				dataType:'json',
				success:function(msg){
					$(msg).each(function(index,item){
						...
						var tiezi = "<li class='ue-clear'>"+
								"<span><img src='${pageContext.request.contextPath}/images/tx.png' height='50px' width='50px'/></span> 发布时间："+newTime+
        ...
        // 弹窗
        function showWindow(notice) {
        	$("#ntitle").text(notice.ntitle);
        	$("#content").text(notice.remark);
            $('#showdiv').show();  //显示弹窗
...
<br>
	...
	<table width="98%" align="center" border="0" cellpadding="3"
		...
		<tr>
			<td background="${pageContext.request.contextPath}/skin/images/frame/wbg.gif" bgcolor="#EEF4EA"
				class='title'><span>通知公告</span><a href='#' style='padding-left: 1260px'></a></td>
		</tr>
		<tr bgcolor="#FFFFFF">
			<td>
    ...
    <table width="98%" align="center" border="0" cellpadding="3"
		...
		<tr>
			<td colspan="3" background="skin/images/frame/wbg.gif" bgcolor="#EEF4EA"
				class='title'>
				<span>员工论坛</span><a href='#' style='padding-left: 1260px'></a>
			</td>
		</tr>
